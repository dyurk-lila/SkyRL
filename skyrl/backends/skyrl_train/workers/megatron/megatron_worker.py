...
class MegatronWorker:
    ...
    def init_configs(
        ...
        _hpat = getattr(provider, "hybrid_override_pattern", None) or getattr(provider, "hybrid_layer_pattern", None)
        if _vpp_size is not None and _vpp_size > 1 and _pp_size > 1 and isinstance(_hpat, str) and "|" not in _hpat:
    ...
    def _maybe_initialize_tp_comm_ub(self, provider) -> None:
        ...
        try:
            ...
            logger.error(
                f"TP-COMM-UB-INIT (lila): initialize_ub FAILED rank={_rank} " f"shape={shape} tp_size={tp_size}: {e!r}"
            )
            raise
