...
class HFModelWrapper(nn.Module):
    ...
    def forward(
        ...
        sparse_support = sample_support_csr_ids is not None
        if enable_sample_support_replay and not dense_support and not sparse_support:
            raise ValueError("sample-support replay is enabled but the microbatch has no recorded support")
        if enable_sample_support_replay and dense_support and sparse_support:
        ...
        support_row_ids_fwd = (
            build_sample_support_row_ids(sample_support_csr_offsets, sequences.shape[1], sequences.device)
            if enable_sample_support_replay and sparse_support
            else None
        )
        support_metadata_fwd = support_row_ids_fwd if sparse_support else support_ids_fwd
        ...
        if self.remove_microbatch_padding:
            with torch.no_grad():
                ...
                if support_metadata_fwd is not None:
                    support_metadata_fwd = support_metadata_fwd.flatten(0, 1).index_select(0, nnz_indices).unsqueeze(0)
        ...
        support_metadata_rolled = (
            torch.roll(support_metadata_fwd, shifts=-1, dims=1) if support_metadata_fwd is not None else None
        )
        ...
        if self.sequence_parallel_size > 1:
            ...
            if support_metadata_rolled is not None:
                support_metadata_rolled, _, _, _ = ulysses_pad_and_slice_inputs(
                    support_metadata_rolled,
                    None,
                    None,
        ...
        if enable_sample_support_replay:
            assert (
                support_metadata_rolled is not None
            ...
            if sparse_support:
                ...
                support_scores = aligned_sample_support_csr_scores(
                    logits_BSV,
                    sequences_rolled,
                    support_metadata_rolled,
        ...
        if compute_entropy:
            if support_entropy is not None and support_entropy_mask is not None:
                ...
                # Packed data is unpacked after entropy; otherwise exclude padding.
                entropy_attention_mask = None if self.remove_microbatch_padding else attention_mask_fwd
                entropy_BS = self.chunked_entropy_from_logits_fn(
                    logits_BSV,
                    requires_grad=entropy_requires_grad,
                    attention_mask=entropy_attention_mask,
