...
- `optimizer_config.optimizer`: Optimizer type. Defaults to `"adam"` (AdamW with decoupled weight decay; there is no separate `"adamw"` token). `"adam"` and `"sgd"` are supported on both the FSDP and Megatron backends. `"lion"`, `"muon"`, and `"soap"` are reachable only on the Megatron backend and require the optional `emerging-optimizers>=0.2` package; the FSDP backend raises `NotImplementedError` for them. Note that `"sgd"` momentum differs by backend: Megatron uses Megatron-core's default momentum (0.9), while FSDP currently builds `torch.optim.SGD` with its default `momentum=0` (vanilla SGD).
