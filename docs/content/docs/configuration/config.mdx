...
<Callout type="warn">
Only the default `optimizer: "adam"` path is exercised by SkyRL's tests and training runs. `"sgd"` and all emerging optimizers (`"lion"`, `"muon"`, `"adaptive_muon"`, `"soap"`, ...) are **wired but not yet convergence-validated** on real GPU training. They are reachable/passed through to the backend, but treat them as experimental and validate convergence before relying on them.
</Callout>

- `optimizer_config.optimizer`: Optimizer type. Defaults to `"adam"` (AdamW with decoupled weight decay; there is no separate `"adamw"` token — `"adamw"` is rejected). `"adam"` and `"sgd"` are the cross-backend core and dispatch natively on both the FSDP and Megatron backends. Any other name is treated as a Megatron-only "emerging" optimizer and is **passed through to Megatron-core**, which is the authority on the supported set (validated at construction time). SkyRL deliberately does not maintain its own allow-list of emerging optimizers, to avoid drifting from Megatron-core. With the optional `emerging-optimizers` extra installed (`emerging-optimizers>=0.2`), `"lion"`, `"muon"`, and `"adaptive_muon"` are reachable; `"soap"` and other names are exposed dynamically by the installed package and are therefore package/version-dependent. The FSDP backend raises `NotImplementedError` for any non-core optimizer. To enable emerging optimizers, install the extra alongside the Megatron backend, e.g. `uv sync --extra megatron --extra emerging-optimizers` (the package is git-installed from [NVIDIA-NeMo/Emerging-Optimizers](https://github.com/NVIDIA-NeMo/Emerging-Optimizers); the PyPI name is an NVIDIA placeholder).
- `optimizer_config.lr`: Learning rate for the optimizer
- `optimizer_config.adam_betas`: Betas for AdamW optimizer. Only consumed by Adam-family optimizers.
- `optimizer_config.sgd_momentum`: Momentum for `optimizer: "sgd"`, consumed by SGD on both backends. Defaults to `0.0` (vanilla SGD), byte-identical to the historical FSDP default; set to `0.9` for classic momentum SGD. This is the one optimizer-specific field promoted to first-class status because it is relevant to both backends. Other optimizer-specific hyperparameters (e.g. for emerging optimizers) are not given dedicated fields; pass them to the Megatron backend via `megatron_config.optimizer_config_kwargs` instead.
...
policy:
  ...
  optimizer_config:
    optimizer: "adam"  # adam=AdamW (default, validated); sgd; emerging (lion/muon/...) are Megatron-only + unvalidated
    lr: 1.0e-6  # Learning rate
    adam_betas: [0.9, 0.999]  # Betas for Adam optimizer
    sgd_momentum: 0.0  # Momentum for optimizer="sgd" (0.0 = vanilla SGD); both backends
...
critic:
  model:
    ...
    lora:
      ...
      init_method: "kaiming" # Initialization method for LoRA layers
  optimizer_config:
    optimizer: "adam"  # Optimizer type (adam=AdamW; sgd; lion/muon/soap are Megatron-only)
    lr: 5.0e-6
    adam_betas: [0.9, 0.999]
    weight_decay: 1e-2
    max_grad_norm: 1.0 # gradient clipping
