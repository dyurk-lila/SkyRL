...
`moe_router_fusion=True` is incompatible with routing replay because TransformerEngine's fused top-k cannot accept precomputed indices. SkyRL rejects this configuration and forces fusion off at runtime if needed.

Each worker logs its replay path once and warns if configured replay receives microbatches without rollout routes.

## Fused routing replay

`trainer.policy.megatron_config.moe_fused_routing_replay=True` serves replay through a fused CUDA kernel with Megatron's `(routing_probs, routing_map)` contract. It supports normalized sigmoid routing, fp32/fp16/bf16 logits, and `2 <= topk <= 32`; unsupported calls fall back to unfused replay. Workers compile the extension before the first forward in a node-local, torch-version-keyed directory.
