...
class TestSubSeqLengths:
    ...
    def test_multiseq_with_tp_and_fp8_aggregate_alignment(self):
        ...
        total_align_size = get_packing_align_size_total(tp_size=4, cp_size=1, fp8_enabled=True)
        seq_len = total_align_size
        input_ids = torch.zeros(batch_size, seq_len, dtype=torch.long)
    ...
    def _reassemble_cp_shards(per_rank_out, params_cp, batch_size, seq_len, cp_size, sub_seq_lengths):
        ...
        for row_idx, row_lens in enumerate(sub_seq_lengths):
            ...
            for seg in range(row_first_sub, row_last_sub_exclusive):
                ...
                copy_len = min(padded_segment_len, seq_len - dest_off)
                output[row_idx, dest_off : dest_off + copy_len] = tmp[:copy_len]
