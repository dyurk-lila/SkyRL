...
from skyrl.backends.skyrl_train.utils.ppo_utils import repopulate_all_registries
...
def _make_cpu_policy_worker() -> PolicyWorkerBase:
    ...
    """
    # PolicyWorkerBase.__init__ resolves the loss via PolicyLossRegistry.get(...).
    # When Ray is initialized (the session-scoped ray_init fixture in
    # tests/backends/skyrl_train/conftest.py), the registry syncs against a Ray
    # actor that an earlier test may have left empty, so the import-time
    # @register_policy_loss decorators are not enough on their own. Repopulate the
    # default policy losses first — this mirrors validate_cfg() at startup
    # (skyrl/train/utils/utils.py), which calls repopulate_all_registries() before
    # any registry lookup.
...
def _run_forward_backward_micro_rl(loss_fn_config):
    ...
    # ``use_kl_loss`` defaults to True (skyrl/train/config/config.py); turn it off here so
    # the RL branch never references ``base_action_log_probs`` (left None on this Experience)
    # and the KL term stays a no-op. ``use_entropy_loss`` defaults off, so the entropy term is
    # a zero tensor; dp_size feeds the grad-sum correction factor.
