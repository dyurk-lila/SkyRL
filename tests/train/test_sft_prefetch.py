...
(``SFTConfig.prefetch_data``, gated additionally by ``SKYRL_SFT_PREFETCH_DATA``).
The prefetched batch MUST be byte-identical to what the synchronous path would
have produced for that same step — including across epoch boundaries, where the
data reshuffles in place.
...
Also unit-tests the ``_BatchPrefetcher`` invariants (mismatch assertion,
...
or with the worktree venv:
  /home/ray/sft_venv/bin/python -m pytest tests/train/test_sft_prefetch.py -v
"""
...
from skyrl.train.sft_trainer import SFTTrainer, _BatchPrefetcher
...
def _make_trainer(cfg: SFTConfig, collator) -> SFTTrainer:
    ...
    # inference-server stack — unneeded for a CPU-only collate test and broken in
    # some envs. The train() loop only reads ``self.cfg`` through
    # ``_torch_profiler_enabled``; force that OFF.
    skyrl_cfg = MagicMock()
    skyrl_cfg.trainer.policy.torch_profiler_config.enable = False
...
def _capture_batches(trainer: SFTTrainer, monkeypatch) -> list[dict]:
    ...
    monkeypatch.setattr(trainer, "_load_and_tokenize", lambda *_a, **_k: None)  # unused
...
def _run_pair(monkeypatch, collator_factory, n_examples, batch_size, num_steps):
    ...
    def _run(prefetch_on: bool):
        if prefetch_on:
            monkeypatch.delenv("SKYRL_SFT_PREFETCH_DATA", raising=False)
        else:
            monkeypatch.setenv("SKYRL_SFT_PREFETCH_DATA", "0")
...
# ---------------------------------------------------------------------------
# Unit tests for the _BatchPrefetcher invariants
# ---------------------------------------------------------------------------


def test_prefetcher_basic_submit_get():
    pf = _BatchPrefetcher(lambda step: f"batch-{step}")
    try:
...
def test_prefetcher_step_mismatch_raises():
    pf = _BatchPrefetcher(lambda step: step)
    try:
        pf.submit(3)
...
def test_prefetcher_clear_drops_inflight():
    pf = _BatchPrefetcher(lambda step: step)
    try:
        pf.submit(7)
        pf.clear()
        ...
        # After clear we can submit again (slot freed).
        pf.submit(8)
        assert pf.get(8) == 8
    finally:
        pf.shutdown()
