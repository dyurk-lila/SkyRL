...
class TestPreprocessPackedRows:
    ...
    def test_mbs2_tp4_multisubseq_rows_pack_correctly(self):
        ...
        total_align_size = get_packing_align_size_total(tp_size, cp_size=1, fp8_enabled=True)
        assert params.cu_seqlens_q.tolist() == [0, 7, 12, 15, total_align_size]
        # The 26 valid tokens share one aggregate FP8/TP-aligned slab.
        assert packed.shape == (1, total_align_size)
        ...
        assert packed[0, 26:].tolist() == [0] * (total_align_size - 26)
    ...
    def test_mbs2_tp4_multisubseq_loss_mask_is_zero_at_padding(self):
        """The collator zeros ``loss_mask`` at every padding slot
        ...
        # The collator MUST zero loss_mask at every pad position
        ...
        for r in range(batch.batch_size):
            ...
            assert (lm[zero_attn_positions] == 0).all(), (
                f"row {r}: loss_mask is non-zero at pad slots "
